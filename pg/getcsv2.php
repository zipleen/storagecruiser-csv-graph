...
if(isSet($_SESSION['fields']))
	...
	*/
	...
	//echo "De: <input type='text' value='2012-06-08 12:00:00' name='de' size='40'/> Ate: <input type='text' value='2012-06-09 11:59:59' name='ate' size='40' />";
	echo "De: <input type='text' value='2012-06-08 21:30:00' name='de' size='40'/> Ate: <input type='text' value='2012-06-09 02:00:00' name='ate' size='40' />";
	//echo "De: <input type='text' value='".$dblink->GetOne("SELECT MIN(data) from data")."' name='de' size='40'/> Ate: <input type='text' value='".$dblink->GetOne("SELECT MAX(data) from data")."' name='ate' size='40' />";
...
function get_csv()
{
	...
	//if ($fp) {
	 ...
	 // fclose($fp);
	  echo "download csv <a href='out.csv'>here</a><br/><br/>";
	...
	if(isSet($_SESSION['fields']))
        {
                ...
                foreach($_SESSION['fields'] as $id=>$data)
                {
                        echo ", ".getValueFor("props", $data['prop']). " - ".getValueFor("devs", $data['dev']) ;
                }
        }
...
switch($_REQUEST['action'])
{
	...
	case 'addlog':
		...
		$rs = $dblink->Execute("select id, SUBSTRING(name, -2), hex_to_int(substr(name, 18, 2) ) as num from devs where name like 'LogicalVolume%' and hex_to_int(substr(name, 18, 2) )>= 0 and hex_to_int(substr(name, 18, 2) )<=27 or ( hex_to_int(substr(name, 18, 2) )>=57 and hex_to_int(substr(name, 18, 2) )<=63) order by num");
		//$rs = $dblink->Execute("select id, SUBSTRING(name, -2), hex_to_int(substr(name, 18, 2) ) as num from devs where name like 'LogicalVolume%' and hex_to_int(substr(name, 18, 2) )>=28  and hex_to_int(substr(name, 18, 2) )<=56 or ( hex_to_int(substr(name, 18, 2) )>=64 and hex_to_int(substr(name, 18, 2) )<=67) order by num");
